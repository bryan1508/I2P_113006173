...
class Animation(Sprite):
    ...
    def stop(self):
        self.playing = False
    #Acc and current frame : |.....|.....|.....|.....|
    #Acc                     0    .25    .5   .75    1
    #frame                      0      1    2     3
    def set_frame(self, frame: int):
        self.current_frame = max(0, min(frame, self.n_keyframes - 1))
        self.accumulator = (self.current_frame / self.n_keyframes) * self.loop
    #to change row (change direction)
    def set_state(self, name: str):
        if self.cur_row != name:
            self.cur_row = name
            self.set_frame(0)  # reset to idle of new direction
    ...
    def update(self, dt: float):
         if self.playing:
            ...
            self.current_frame = int((self.accumulator / self.loop) * self.n_keyframes)
    ...
    def draw(self, screen: pg.Surface, camera: Optional[PositionCamera] = None):
        ...
        if camera:
            screen.blit(frame, camera.transform_rect(self.rect))
        else:
