...
class ChatOverlay(UIComponent):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            self._font_msg = pg.font.Font(font_path, 18)
            self._font_input = pg.font.Font(font_path, 20)
        except Exception:
            self._font_msg = pg.font.SysFont("Arial", 18)
    ...
    def draw(self, screen: pg.Surface) -> None:
        ...
        x = 10  # left offset
        log_h = 180  # height of chat log area
        input_h = 28
        spacing = 10

        # top of log box (leave room for input bar under it)
        log_y = sh - (log_h + input_h + spacing + 6)
        ...
        if msgs:
            ...
            screen.blit(bg, (x, log_y))

            # last N messages, bottom-aligned
            visible = msgs[-8:]
            line_h = self._font_msg.get_linesize()

            total_height = len(visible) * line_h
            bottom_padding = 8
            draw_y = log_y + container_h - bottom_padding - total_height
